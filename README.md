...
Substituting the full augmented ansatz $\tilde{\mathbf{w}} = \mathbf{w}_{\text{ref}} + \mathbf{V}\mathbf{q} + \bar{\mathbf{V}}\bar{\mathbf{q}}$ into the governing PDE yields:
...
where $\bar{\mathbf{q}} = \mathcal{N}(\mathbf{q}) = W_2 \tanh(W_1\mathbf{q} + \mathbf{b}_1) + \mathbf{b}_2 + W_3(\mathbf{q} \odot \mathbf{q})$. The orthogonality $\mathbf{V}^T\bar{\mathbf{V}} = 0$ ensures the primary and secondary subspaces are decoupled (the network correction enters the projected residual without violating the primary Galerkin equations).
